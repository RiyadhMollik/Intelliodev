...
export default function ChromaGrid({
  ...
  const handleMove = (e: React.PointerEvent<HTMLDivElement>) => {
    const r = (rootRef.current as HTMLDivElement).getBoundingClientRect()
  ...
  return (
    <div
      ref={rootRef}
      className={`${styles.chromaGrid} ${className} border-b-indigo-950`}
      style={{
        // @ts-ignore custom props for CSS vars
        '--r': `${radius}px`,
        '--cols': columns,
        '--rows': rows,
      }}
      ...
      {data.map((c, i) => (
        ...
        >
          <div className={styles.chromaImgWrapper}>
            <Image src={c.image} alt={c.title} loading="lazy" width={500} height={500} />
          </div>
